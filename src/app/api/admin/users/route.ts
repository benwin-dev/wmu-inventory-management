...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json()) as { email?: string; role?: string; cafe_id?: number | null };
  ...
  try {
    const result = await pool.query<{ id: number; email: string; role: string; cafe_id: number | null; cafe_name: string | null; created_at: string }>(
      `INSERT INTO user_roles (email, role, cafe_id) VALUES ($1, $2, $3)
       ON CONFLICT (email) DO UPDATE SET role = EXCLUDED.role, cafe_id = EXCLUDED.cafe_id
       RETURNING id, email, role, cafe_id, created_at`,
      [email, role, resolvedCafeId],
    );
